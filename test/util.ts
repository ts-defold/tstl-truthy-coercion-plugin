...
// @ts-ignore
import { createVirtualProgram } from "typescript-to-lua/dist/transpilation";
...
const luaLib = fs.readFileSync(path.resolve(__dirname, "../node_modules/typescript-to-lua/dist/lualib/lualib_bundle.lua"), "utf8");
...
export abstract class TestBuilder {
    ...
    public getMainJsCodeChunk(): string {
        ...
        // @ts-expect-error - type not exported by tstl
        )?.js; 
...
const createTestBuilderFactory =
    ...
    (...args: [string] | [TemplateStringsArray, ...any[]]): T => {
        ...
        const runner = new builder(tsCode);
        runner.setOptions({
            luaPlugins: [
                {
